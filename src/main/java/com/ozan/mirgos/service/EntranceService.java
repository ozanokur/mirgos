...
public class EntranceService {
    ...
    @Transactional
    public void processCourierLocation(Long courierId, Point location, LocalDateTime courierLocationTime) {
        ...
        LocalDateTime thresholdTime = courierLocationTime.minusMinutes(thresholdMinutes);
        ...
        for (Store store : nearbyStores) {
            ...
            boolean alreadyExists = entranceRepository.findRecentEntrance(
                    courierId, store.getId(), thresholdTime).isPresent();

            if (!alreadyExists) {
                Entrance entrance = new Entrance();
                entrance.setCourierId(courierId);
                entrance.setStoreId(store.getId());
                entrance.setEventTime(courierLocationTime);
    ...
    public List<Entrance> getEntrances(Long courierId, LocalDateTime from) {
        return entranceRepository.findEntranceByCourierIdAndTime(
                courierId, from);
    }
}
